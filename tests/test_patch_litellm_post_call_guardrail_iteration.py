...
SCRIPT_PATH = (
    ...
    / "scripts"
    / "patch_litellm_post_call_guardrail_iteration.py"
)


class PatchLiteLLMPostCallGuardrailIterationTests(unittest.TestCase):
    def _load_patch_module(self):
        spec = importlib.util.spec_from_file_location(
            "patch_litellm_post_call_guardrail_iteration",
            SCRIPT_PATH,
        )
    ...
    def test_nonmatching_guardrail_does_not_stop_post_call_iteration(self) -> None:
        patch_module = self._load_patch_module()
        with tempfile.TemporaryDirectory() as directory:
            target = Path(directory) / "custom_guardrail.py"
